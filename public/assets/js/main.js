...
// categorylist----------------------------------
    $('.main-menu ul.list-menu li.item-list-menu .sub-menu li.list-item-children:first-child').addClass('active');
    $('.main-menu ul.list-menu li.item-list-menu .sub-menu li.list-item-children').on('mouseenter', function () {
...
//    verify-phone-number------------------------
        ...
        text: '<span class="day">%s</span><span class="hour">%s</span><span>: %s</span><span>%s</span>',
            ...
            $countdownOptionEnd.html("<a href='' class='link-border-verify form-account-link'>ط§ط±ط³ط§ظ„ ظ…ط¬ط¯ط¯</a>");
        }
        });
        }
...
if (location.pathname == '/login') {
    passElem.addEventListener('click', () => {
        if (statusElem) {
            inputElem.type = 'text';
            passElem.className = 'fa fa-eye-slash pass-status';
        } else {
            inputElem.type = 'password';
            passElem.className = 'fa fa-eye pass-status';
        }
...
function changeQuantity(event, Id){
    let formElem = document.querySelector('#submit-form'+Id)
    let quantity = document.querySelectorAll('.quan'+Id) // div quantity & hidden input

    let result = Number(quantity[0].innerHTML)

    if (event.target.className === 'fa fa-plus'){
        result += 1
        changeQuantityValue(result)
    } else if(event.target.className === 'fa fa-minus'){
        result -= 1
        changeQuantityValue(result)
    }

    function changeQuantityValue(result) {
        if (result >= 1) {
            quantity[0].innerHTML = result
            quantity[1].value = result
